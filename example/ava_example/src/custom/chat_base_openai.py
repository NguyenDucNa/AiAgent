...
class BaseChatOpenAI(BaseChatModel):
    ...
    def fake_generate(self, last_message: AIMessage) -> Iterator[dict]:
        for i in range(0, len(last_message.content), 5):
            fake_chunk = {
                "id": last_message.id,
                "choices": [
                    {
                        "delta": {
                            "content": last_message.content[i:i+5],
                            "function_call": None,
                            "refusal": None,
                            "role": None,
                            "tool_calls": None,
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        if isinstance(last_message, AIMessage) and not last_message.tool_calls: 
            response = self.fake_generate(last_message)
            is_first_chunk = True
            for chunk in response:
                # print("TTFT: ", time.time() - start_time, flush=True)
